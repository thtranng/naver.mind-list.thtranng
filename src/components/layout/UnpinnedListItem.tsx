...
interface UnpinnedListItemProps {
  listData: UserList;
}
...
export function UnpinnedListItem({ listData }: UnpinnedListItemProps) {
  ...
  return (
    <div className="relative list-item group">
      {/* Main Item - Horizontal Layout */}
      <div
        onClick={handleClick}
        className={`w-full flex items-center p-3 rounded-lg transition-all duration-200 cursor-pointer ${
          state.selectedListId === listData.id ? 'border-l-4 border-blue-500' : ''
        }`}
        style={{
          backgroundColor: state.selectedListId === listData.id
            ? `${listData.color}20`
            : (listData.color ? `${listData.color}08` : 'transparent')
        }}
        onMouseEnter={(e) => {
          if (state.selectedListId !== listData.id) {
            e.currentTarget.style.backgroundColor = listData.color ? `${listData.color}15` : '#f3f4f6';
          }
        }}
        onMouseLeave={(e) => {
          if (state.selectedListId !== listData.id) {
            e.currentTarget.style.backgroundColor = listData.color ? `${listData.color}08` : 'transparent';
          }
        }}
      >
        {/* Left: Icon */}
